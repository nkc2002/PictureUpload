...
// Hàm tạo HTML
function generateHTML(error, success, files) {
    ...
    const imageGallery = files.length > 0 ? `
...
app.get('/', (req, res) => {
    ...
    res.send(generateHTML(error, success, files));
});
...
app.post('/upload', (req, res) => {
    ...
    uploadMultiple(req, res, (err) => {
        if (err instanceof multer.MulterError) {
            ...
            // Lỗi khác (bao gồm lỗi từ fileFilter)
            return res.redirect('/?error=' + encodeURIComponent(err.message));
        }
        
        // Kiểm tra có file nào được upload không
        if (!req.files || req.files.length === 0) {
            return res.redirect('/?error=' + encodeURIComponent('Vui lòng chọn ít nhất một file ảnh PNG!'));
        }
        
        // Upload thành công
        const uploadedFiles = req.files.map(file => file.filename).join(',');
        res.redirect('/?success=true&files=' + uploadedFiles);
    });
});
